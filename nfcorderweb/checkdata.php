...
<?php header('refresh: 5;url="checkdata.php"') ?>
...
<?php
			...
			$nowclass=mysql_fetch_row($us);  //取得現在課程
			...
			{
				$sql = "SELECT * FROM tbl_user where username = '$data1[1]'";  //從tb1_user找id
				$us2 = mysql_query($sql); //傳回成功  代表有資料
				$data=mysql_fetch_row($us2);  //取得id 
				
				 echo "<tr><td>$nowclass[1]</td>";
				 echo "<td><img src=\"./upload/$data[3]\" height=\"100\" width=\"100\"></td>";
				 echo "<td>$data[1]</td>";
				 echo "<td>$data[2]</td>";
				 echo "<td>$data1[3]</td></tr>";
			}				
?>
   </tbody>
 </table>    
